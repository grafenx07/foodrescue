...
export default function LoginPage() {
  ...
  const demoAccounts = [
    { role: 'Donor', email: 'sarah@restaurant.com' },
    { role: 'Receiver', email: 'ananya@ngo.com' },
    { role: 'Volunteer', email: 'harsh@volunteer.com' },
  ];
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-green-600 text-white font-semibold py-3 rounded-xl hover:bg-green-700 transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
            >
          ...
          <div className="mt-8 bg-blue-50 rounded-xl p-4">
            ...
            {demoAccounts.map(acc => (
              <button
                ...
                className="block text-xs text-blue-700 hover:text-blue-900 mb-1"
              >
                <strong>{acc.role}:</strong> {acc.email}
              </button>
            ))}
            <p className="text-xs text-blue-600 mt-1">Password: <strong>password123</strong></p>
          </div>
        </div>
      </div>
    </div>
